...
export const About = ({ info }: { info: AboutInfo }) => {
  const { t, lang, isRtl } = useLang();

  const getContent = (key: string) => {
    if (!info) return "";
    const suffix = lang.charAt(0).toUpperCase() + lang.slice(1);
    return (info as any)[`${key}${suffix}`] || (info as any)[key] || (t.about as any)[key] || "";
  };

  const profileImg = info?.profileImage || "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=1200";
  const resumeUrl = info?.resumeUrl || "";

  // Fix for invalid URLs to prevent router crash
  const handleDownload = (e: React.MouseEvent) => {
    if (!resumeUrl || resumeUrl === "#") {
        e.preventDefault();
        alert(isRtl ? "ملف الأعمال غير متوفر حالياً" : "Portfolio file is not available yet.");
    }
  };

  return (
    <div className="max-w-6xl mx-auto px-6 py-40 min-h-screen" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="grid md:grid-cols-2 gap-16 lg:gap-24 mb-48 items-center">
        ...
        <motion.div initial={{ opacity: 0, x: isRtl ? -30 : 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
            <span className="text-[10px] uppercase tracking-[0.6em] text-[#d4a373] mb-6 block font-black">{t.about.bioTitle}</span>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative py-24 px-8 md:px-20 bg-white/[0.02] border border-white/5 rounded-[4rem] text-center mb-48">
